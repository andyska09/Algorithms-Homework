...
size_t countVisited(const std::vector<State> &state)
{
    size_t count = 0;
...
size_t bfs(const Graph &G, Vertex u, std::vector<Vertex> &P, std::vector<size_t> &D)
{
    P[u] = ROOT;
    D[u] = 0;
    std::vector<State> state(G.vertices(), UNFOUND);
    std::queue<Vertex> q;
    q.push(u);
    state[u] = OPEN;
    while (q.size() > 0)
    ...
    return countVisited(state); // therefore count the number of closed vertices
}
